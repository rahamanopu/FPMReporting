...
$(function () {
    ...
    $("#materialcode").autocomplete({ 
        source: function(request, response){ //console.log(request);
            ...
            //alert("search=" + request.term + "&businesscode=" + businesscode + "&materialtypecode" + materialtypecode);
            $.ajax({
                type: "POST",
...
function validateform(){
    ...
    if(duplicatateindex >= 0){
        ...
        table[duplicatateindex]['buffersize'] = buffersize;    
    }else{
    ...
    $.ajax({
        ...
        success: function (res) {    
            ...
            if(res){
                ...
                $("#nb_category").html(res[0]['BufferSize']);
                $("#nb_effectivedate").html(res[0]['BFEffectiveDate']);
                $("#nb_coolingperiod").html(res[0]['CoolingPeriod']);
            }else{
                $("#notificationboard").html('');
                $("#notificationboard").html('No buffer size found.');
            }
        },
        error: function (msg) {
           console.log(msg);         
...
function doChangeStatus(divname){
