...
function doLoadExcelData(exceldata){
    if(exceldata && exceldata.length > 0){ 
        ...
        for(i = 0; i < exceldata.length; i++){
            ...
            plantlocation = exceldata[i]['locationcode'];
            ...
            table.push({"materialcode": material,"businesscode": business
                ...
                , "minimumorderqty": exceldata[i]['minimumorderqty']});
        }
        loaddata();
    }
}
...
$(function () {
    ...
    $( "#effectivedate" ).keydown(function (e){
        if(e.keyCode == 13){
            $("#minimumorderqty").focus();
        }
    })
    $( "#minimumorderqty" ).keydown(function (e){
        if(e.keyCode == 13){
            $("#addbutton").focus();
        }
    })
...
function validateform(){
    ...
    var minimumorderqty = $("#minimumorderqty").val();
    ...
    if(duplicatateindex >= 0){
        ...
        table[duplicatateindex]['minimumorderqty'] = minimumorderqty;    
    }else{
...
function loaddata(){
    ...
    for(i = 0; i < table.length; i++){
        $('#datagrid tbody').append('<tr ondblclick="loadGridData(\''+ table[i]['materialcode'] + '\')">\
            ...
            <input type="hidden" name="minimumorderqty[]" value="'+ table[i]['minimumorderqty'] + '">\n\
...
function loadGridData(materialinfo){
    for(i = 0; i < table.length; i++ ) {
       if(table[i]['materialcode'] == materialinfo){
            ...
            var minimumorderqty = $("#minimumorderqty").val(table[i]['minimumorderqty']);
       }
       $("#addbutton").prop('disabled', false);
       $("#materialcode").prop('readonly', true);
    }
    ...
    $.ajax({
        ...
        success: function (res) {    
            ...
            if(res){
                ...
                $("#nb_category").html(res[0]['MinimumOrderQty']);
                $("#nb_effectivedate").html(res[0]['MOQEffectiveDate']);
                $("#nb_coolingperiod").html(res[0]['CoolingPeriod']);
            }else{
                $("#notificationboard").html('');
                $("#notificationboard").html('No Minimum Order Quantity Found.');
            }
        },
        error: function (msg) {
           console.log(msg);         
